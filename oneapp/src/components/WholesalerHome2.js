...
class WholesalerHome extends Component {
  constructor(props) {
    ...
    const { user } = cart;
    console.log("In main constructor--WHOLESALER ", user.id);
    this.state = {
      wholesalerItem: [],
      flag: false,
      id: 0,
...
ApiService.addItemWholesaler(addedProduct)
      .then((response) => {
        const { data } = response;
...
handleDelete = (id) => {
  console.log(id);
    ...
    ApiService.deleteProductwholesaler(id, userid)
      .then((response) => {
        const { data } = response;
        console.log(response);
        console.log(data);
...
handleAdd = () => {
  const { error, isLoaded, wholesalerItem, handleAdd, flag, handleUpdate } =
    this.state;
...
handleSubmit = (id) => {
  ...
  ApiService.updateQuantitywholesaler(id, quant)
    .then((response) => {
      const { data } = response;
      console.log(response);
      console.log(data);
...
// ----------------------------------------RetailerCopy ends--------------------------
  ...
  render() {
    const { error, isLoaded, wholesalerItem, handleAdd, flag } = this.state;
    if (error) {
...
export default connect(mapStateToProps, mapActionsToProps)(WholesalerHome);
