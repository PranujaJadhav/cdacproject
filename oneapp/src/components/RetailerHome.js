...
class RetailerHome extends Component {
  ...
  handleAdd = () => {
    const { error, isLoaded, retailerItem, handleAdd, flag, handleUpdate } =
      this.state;
  ...
  render() {
    const { error, isLoaded, retailerItem, handleAdd, flag } = this.state;
    if (error) {
...
export default connect(mapStateToProps, mapActionsToProps)(RetailerHome);
