...
class retailerlistadmin extends Component {
  constructor(props) {
  ...
  getretailerListforAdmin = () => {
    ...
    ApiService.getretailerListforAdmin()
      .then((response) => {
        const { data } = response;
        console.log(response);
        console.log(data);
  ...
  render() {
    const { error, isLoaded, retailslist, handleAdd, flag } = this.state;
    if (error) {
...
export default connect(mapStateToProps, mapActionsToProps)(retailerlistadmin);
